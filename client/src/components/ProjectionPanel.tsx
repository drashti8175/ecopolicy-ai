    ...
    <Card className="border-blue-100">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <TrendingDown className="h-5 w-5 text-blue-600" /> Future Climate Projection
        </CardTitle>
        <CardDescription>2026 – 2040 · Business-as-usual vs. proposed policy</CardDescription>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          <div className="flex h-48 items-center justify-center text-slate-400 text-sm">Loading projection...</div>
        ) : (
          <>
            <div className="mb-4 flex gap-4">
              <div className="rounded-lg bg-blue-50 px-4 py-2 text-center">
                <p className="text-xs text-slate-500">CO₂ Avoided by 2040</p>
                <p className="text-xl font-bold text-blue-700">{co2Saved} MT</p>
              </div>
            </div>

            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">CO₂ Emissions (MT)</p>
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={chartData}>
                ...
                <Line type="monotone" dataKey="With Policy CO₂" stroke="#34d399" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>

            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2 mt-4">AQI Trend</p>
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={chartData}>
                ...
                <Line type="monotone" dataKey="With Policy AQI" stroke="#60a5fa" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </>
        )}
